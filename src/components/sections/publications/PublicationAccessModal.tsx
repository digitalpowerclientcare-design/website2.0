...
export function PublicationAccessModal({
  ...
  const submitted = successEmail !== null;
    ...
    >
      ...
      >
        <button
          type="button"
          onClick={onClose}
          className="absolute top-4 right-4 flex h-9 w-9 items-center justify-center rounded-full text-[var(--ink-muted)] transition-colors hover:bg-[var(--surface)] hover:text-[var(--ink)]"
          aria-label="Close"
        >
          <X size={18} />
        </button>
        ...
        {submitted ? (
          ...
          <>
            ...
            >
              ...
              <button
                type="submit"
                className="mt-2 w-full rounded-full bg-[var(--indigo)] py-3.5 text-[15px] font-medium text-white transition-colors hover:bg-[var(--indigo-deep)]"
              >
                {PUBLICATION_ACCESS_COPY.submitLabel}
              </button>
